...
// Removing the element at index `i` splits countFree into ps[i]'s contribution
// plus the count over the list with `i` removed. Proved purely from the
// concat-homomorphism on ps == ps[..i] + [ps[i]] + ps[i+1..] — this is the
// "remove-at-index" step the permutation induction needs.
lemma CountFreeRemoveAt(ps: seq<Participant>, i: int, s: int)
  requires 0 <= i < |ps|
  ensures countFree(ps, s)
       == (if freeAt(ps[i], s) then 1 else 0) + countFree(ps[..i] + ps[i + 1..], s)
{
  assert ps == ps[..i] + [ps[i]] + ps[i + 1..];
  countFreeConcat_ensures(ps[..i] + [ps[i]], ps[i + 1..], s);
  countFreeConcat_ensures(ps[..i], [ps[i]], s);
  countFreeConcat_ensures(ps[..i], ps[i + 1..], s);
  assert [ps[i]][1..] == [];   // countFree([ps[i]], s) == (freeAt(ps[i], s) ? 1 : 0)
}
  ...
  ensures (countFree(xs, s) == countFree(ys, s))
{
  ...
  // each side via CountFreeRemoveAt / the homomorphism, then recurse on the
  ...
  if (|xs| == 0) {
    ...
    CountFreeRemoveAt(ys, i, s);
    countFreeConcat_ensures([x], xs[1..], s);
    assert [x][1..] == [];
...
lemma heatmapPermInvariant_ensures(a: Event, b: Event)
  ...
  ensures forall s: int :: ((0 <= s) ==> (s < a.numSlots) ==> (heatmap(a)[s] == heatmap(b)[s]))
{
  // --- proof: each heatmap cell is the countFree at that slot (heatmap_ensures);
  // countFreePerm makes those equal slot-by-slot under the permutation. ---
  heatmap_ensures(a);
  heatmap_ensures(b);
  forall s: int | 0 <= s < a.numSlots
    ensures heatmap(a)[s] == heatmap(b)[s]
  {
