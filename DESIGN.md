...
> **Built so far (the app).** A pure **React + Vite SPA**, light-themed, local-first, with a deliberately **transport-agnostic seam**: `src/store.js` is the only module that imports `applyOp` (`dispatch(op)` = `applyOp` + persist + notify, over a monotonic LWW clock), and `src/useQuorum.js` is the only caller of `heatmap`/`isBest`/`availableAtLeast`/`maxCount` — so no scheduling math lives in JSX. Today the store is local (in-memory + `localStorage`); a future `RemoteStore` (WebSocket → the Durable Object above) implements the same `{getSnapshot, subscribe, dispatch}` interface with **no UI rewrite**. The cell↔slot mapping goes through the verified `src/grid.ts`; the NDJSON export uses the verified `sparsify`. UI is `src/App.jsx` (event creation in **two modes — a Sunday-start month calendar for specific dates, or day-of-week chips for recurring availability**; single grid with Paint/Group toggle, participant chips, `≥ k` threshold) + `src/components/Grid.jsx`. The grid model is `{kind: "dates" | "weekdays", cols, …}` — both modes are pure shell labeling over the same flat slot indices, so `domain.ts`/`grid.ts` are untouched by the distinction. Runtime-checked by `test/smoke.mjs` (`npm test`).
