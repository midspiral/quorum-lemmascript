...
What is **not** verified (the trust boundary, stated honestly): the React UI, the WebSocket/storage I/O, the slot-index ⟷ wall-clock-time labeling (timezones), and abuse/rate-limiting. The core reasons in abstract slot indices; the shell maps those to real calendar times for display.
